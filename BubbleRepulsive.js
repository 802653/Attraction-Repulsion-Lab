...
function BubbleRepulsive(x, y, dx, dy, rad, clr, force){
...
BubbleRepulsive.prototype.run = function(){
...
BubbleRepulsive.prototype.checkOverlapping = function(){
    this.isOverlapping = false;//  default color
    this.clr =  "rgba(137, 196, 244,255)"
    let b = game.bubbles;
    for(let i = 0; i < b.length; i++){ // for all the bubbles
       if(this !== b[i]){   // if not this bubble
         ...
         if(d < this.rad + b[i].rad + this.force){
			...
			b[i].velocity.setDirection(this.location.angleBetween(b[i].location));
...
BubbleRepulsive.prototype.render = function(){
    let ctx = game.ctx;
    // color depends on whether this bubble overlaps any oher bubble
	if(!this.isOverlapping) {
		ctx.strokeStyle = "rgba(137, 196, 244,10)"//this.clr;
	}
	else {
		ctx.strokeStyle = "rgba(137, 196, 244,10)"//this.clr;
	}
...
BubbleRepulsive.prototype.update = function(){
    //if(!game.gamePaused){
...
BubbleRepulsive.prototype.checkEdges = function(){
